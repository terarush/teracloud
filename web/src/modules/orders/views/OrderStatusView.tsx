  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4 sm:px-6 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="text-center space-y-3">
        {isPaid ? (
          <div className="inline-flex items-center justify-center p-3.5 bg-emerald-500/10 text-emerald-500 rounded-2xl ring-1 ring-emerald-500/20 shadow-inner">
            <CheckCircle2 className="size-10 stroke-[2.2]" />
          </div>
        ) : isFailed ? (
          <div className="inline-flex items-center justify-center p-3.5 bg-destructive/10 text-destructive rounded-2xl ring-1 ring-destructive/20 shadow-inner">
            <AlertCircle className="size-10 stroke-[2.2]" />
          </div>
        ) : (
          <div className="inline-flex items-center justify-center p-3.5 bg-amber-500/10 text-amber-500 rounded-2xl ring-1 ring-amber-500/20 shadow-inner animate-pulse">
      ...
      <div className="bg-card border border-border/80 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Step 1: Payment */}
          <div className={`p-4 rounded-xl border transition-all ${isPaid ? "bg-emerald-500/5 border-emerald-500/30" : "bg-muted/30 border-border"}`}>
            <div className="flex items-center gap-3">
              <div className={`size-7 rounded-full flex items-center justify-center text-xs font-bold ${isPaid ? "bg-emerald-500 text-white" : "bg-muted-foreground/30 text-muted-foreground"}`}>
      ...
      {items.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {items.map((item) => (
              <div key={item.id} className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-border">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <Link
            to="/app/containers"
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/90 shadow-md hover:shadow-lg transition-all"
          >
